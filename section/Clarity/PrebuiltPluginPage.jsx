...
function SectionBlock({ section }) {
  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-[#080808] p-5 sm:p-6">
      ...
      {hasParagraphs && (
        <div className="mt-3 space-y-3">
          {section.paragraphs.map((paragraph, index) => (
            <p
              key={index}
              className="w-full text-justify font-normal text-sm leading-relaxed text-white/85 sm:text-[13px] md:text-[16px]"
            >
              {paragraph}
            </p>
          ))}
...
export default function PrebuiltPluginPage({ page }) {
  ...
  return (
    <div className="bg-black">
      ...
      <section className="px-4 pt-6 pb-4 sm:px-6">
        <div className="mx-auto max-w-6xl">
          <p className="w-full text-justify text-sm font-semibold leading-relaxed text-white sm:text-[13px] md:text-[16px]">
            {page.summary}
          </p>
        </div>
      </section>
      ...
      <section className="px-4 pb-10 sm:px-6 sm:pb-12 md:pb-14">
        <div className="mx-auto max-w-6xl space-y-4">
          ...
          {page.closing && (
            <div className="rounded-2xl border border-[#ff403a]/35 bg-gradient-to-br from-[#ff403a]/10 to-transparent p-5 sm:p-7">
              <h4 className="text-xl font-semibold text-white sm:text-2xl">
                {page.closing.title}
              </h4>
              <p className="font-normal mt-3 w-full text-justify text-sm leading-relaxed text-white/85 sm:text-[13px] md:text-[16px]">
                {page.closing.text}
              </p>
            </div>
          )}
